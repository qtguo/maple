...
const int VectorDefaultSize = 64;
...
struct iMap {
  ...
  void initialize(int n) {
    ...
    m_data = new _T[m_num];
    for (int i = 0; i < m_num; ++i) m_data[i] = 1073741834;
    cur = 0;
  }
  void clean() {
  ...
  bool notexist(int p) { return m_data[p] == 1073741834; }
  bool exist(int p) { return !(m_data[p] == 1073741834); }
...
struct iHeap {
  ...
  int head() { return m_data[0].key; }

  void smart_clean(vector<int>& clean_helper) {
    m_data.clean();
    for (auto element : clean_helper) pos.erase(element);
  }

  void clean() { pos.occur.clean(); }
  ...
  void down(int p) {
    ...
    for (int i; p < (int)m_data.m_num; p = i) {
      ...
      if (p * 2 + 1 < (int)m_data.m_num && m_data[p * 2 + 1].value < m_data[p].value)
        ...
        i = p;
      if (p * 2 + 2 < (int)m_data.m_num && m_data[p * 2 + 2].value < m_data[i].value) i = p * 2 + 2;
      if (i == p) break;
...
struct bArray {
  ...
  inline uint32_t* getData(int index) { return arr + (index >> 5); }
  inline bool getBool(int index) { return (arr[index >> 5]) & (1 << (index & 0x1F)); }
  inline void setTrue(int index) { arr[index >> 5] |= (1 << (index & 0x1F)); }
  inline void setFalse(int index) {
    uint32_t flag = ~(1 << (index & 0x1F));
    arr[index >> 5] &= flag;
  }
};
#endif